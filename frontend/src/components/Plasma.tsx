...
export const Plasma: React.FC<PlasmaProps> = ({
  ...
  useEffect(() => {
    ...
    const effectiveRenderScale = isMobile ? Math.min(renderScale, 0.4) : renderScale;
    const effectiveMaxDpr = isMobile ? Math.min(maxDpr, 1.25) : maxDpr;
    const effectiveFps = isMobile ? Math.min(targetFps, 45) : targetFps;
    const effectiveIterations = isMobile ? Math.min(iterations, 45) : iterations;
