...
with st.sidebar:
    ...
    st.info("💡 **Core Innovation**:\n- 🧠 Context (Ask Less, Know More)\n- 🔗 Evidence-Linked Facts\n- ⚠️ Contradiction & Safety Red Flags\n- 🌐 HL7 FHIR R4 Ready")
...
if st.session_state.role == "patient":
    st.title("🗣️ Patient Zero-Form Intake")
    st.caption("Multilingual, low-typing intake for Indian patients. AI collects symptoms and links to medical records.")

    tab1, tab2, tab3 = st.tabs(["1. Spoken Intake", "2. Adaptive Q&A (Ask Less, Know More)", "3. Document Upload & OCR"])
...
# --- DOCTOR ROLE VIEW ---
    ...
    view_mode = st.radio("Display Mode:", ["🔗 EVIDENCE VIEW (Core USP — Clickable Citations)", "📋 SUMMARY VIEW (Clean Case Sheet)", "⏱️ CHRONOLOGICAL TIMELINE", "🌐 FHIR R4 JSON BUNDLE"], horizontal=True)

    if "EVIDENCE VIEW" in view_mode:
    ...
    elif "SUMMARY VIEW" in view_mode:
        st.markdown("""
        ### Clean Clinical Case Sheet Draft
        *NOTICE: AI-generated clinical draft — requires clinician verification. Not a diagnosis.*

        **1. CHIEF COMPLAINT:** Central retrosternal chest pain for 3 days, intermittent breathlessness.
        **2. HISTORY OF PRESENT ILLNESS:** 58-year-old male with acute chest pressure on mild exertion.
        **3. PAST HISTORY:** Type 2 Diabetes Mellitus (2024), Essential Hypertension (2025).
        **4. MEDICATIONS:** Metformin 500mg BD (Active), Amlodipine 5mg OD, Aspirin 75mg (Discrepancy noted).
        **5. ALLERGIES:** Penicillin (Cutaneous rash).
        **6. RED FLAGS:** Cardiopulmonary symptom cluster alert.
        """)

    elif "TIMELINE" in view_mode:
    ...
    elif "FHIR" in view_mode:
        st.markdown("### 🌐 HL7 FHIR R4 Bundle Export (ABDM Interoperability Ready)")
        fhir_bundle = {
            "resourceType": "Bundle",
        ...
        st.download_button(
            label="⬇️ Download FHIR R4 Bundle (.json)",
            data=json_str,
